...
const ConvertEnquiry = () => {
  ...
  const apiRequest = useCallback(
    async (endpoint, method = "GET", body = null, signal = null) => {
  ...
  const fetchEnquiries = useCallback(
    async (signal) => {
      try {
        setLoading(true);
        setError("");
        const data = await apiRequest("/enquiry/all", "GET", null, signal);
  ...
  const validateForm = () => {
    if (!selectedEnquiry) return "Please select an enquiry first.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiRequest("/customer/convert-enquiry", "POST", payload);

      setSuccess(
        "✅ Enquiry converted successfully! A password setup link has been sent to the customer's email."
      );
  ...
  return (
    <div className="convert-container">
      <h1>Enquiry Conversion Desk</h1>

      {error && <div className="error">{error}</div>}
      {success && <div className="success">{success}</div>}

      <div className={`dashboard-workspace ${selectedEnquiry ? "split-active" : ""}`}>
        {/* LEFT PANEL - same as before */}
        <div className="enquiries-panel">
          <h2>Pending Applications ({enquiries.length})</h2>
          {loading ? (
            <div className="loading">Loading enquiries...</div>
          ) : enquiries.length === 0 ? (
            <div className="no-data">No pending enquiries found.</div>
          ) : (
            <div className="enquiry-cards">
              {enquiries.map((enquiry) => (
                <div
                  key={enquiry._id}
                  className={`enquiry-card ${selectedEnquiry?._id === enquiry._id ? "selected" : ""}`}
                ...
                >
                  ...
                  {enquiry.panNumber && (
                    <p><strong>PAN:</strong> <span className="badge-pan-ref">{enquiry.panNumber}</span></p>
                  )}
                  <div className="badge-service">{enquiry.ourServices} • {enquiry.serviceType}</div>
                </div>
              ))}
            </div>
          )}
        ...
        {/* RIGHT PANEL - UPDATED (no password field) */}
        {selectedEnquiry && (
          <div className="conversion-panel">
            <h2>Process Account Conversion</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="action-buttons">
                <button type="submit" className="btn-submit" disabled={submitting}>
                  {submitting ? "Converting..." : "Generate Active Account"}
                </button>
